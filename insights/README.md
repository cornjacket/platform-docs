...
| [Propagate Async Server Errors to Main](development/008-propagate-async-server-errors.md) | Development | Goroutines that can fail fatally must have a path to trigger process shutdown — silent partial failures are worse than crashes |
| [Process Liveness Is Not Health](architecture/004-process-liveness-is-not-health.md) | Architecture | Every service needs HTTP health checks — including background workers — because a running process can be deadlocked, disconnected, or non-functional |
